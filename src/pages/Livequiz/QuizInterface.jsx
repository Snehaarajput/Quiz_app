...
const Quiz = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://quizee-backend-0ltq.onrender.com/quiz/${id}`);
  ...
  const handleSubmitQuiz = async () => {
    try {
      ...
      const response = await axios.post(
        `https://quizee-backend-0ltq.onrender.com/quiz/${id}/take`,
        ...
        <div className="options-container">
          {questions[currentQuestion]?.options.map((option, index) => (
            ...
            >
              {option.optionText}
              {option.optionUrl && (
                <img src={option.optionUrl} alt="" width="30px" className="Image-input"/>
              )}
            </button>
          ))}
        </div>
